...
const schoolClasses: string[] = ['Nursery', 'LKG', 'UKG', ...Array.from({ length: 12 }, (_, index) => `Class ${index + 1}`)];
const seniorSchoolClasses: string[] = Array.from({ length: 12 }, (_, index) => `Class ${index + 1}`);
...
export function getSubjects(boardId: string, className: string = ''): string[] {
  ...
  if (className === 'Nursery' || className === 'LKG' || className === 'UKG' || (!isNaN(classNumber) && classNumber <= 5)) return subjectGroups.schoolJunior;
  if (!isNaN(classNumber) && classNumber >= 11) return subjectGroups.schoolSenior;
