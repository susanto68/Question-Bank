...
function Panel({ children, onClose }) {
  ...
  return createPortal(
    ...
    >
      <motion.div
        ref={panelRef}
        className="ml-auto max-h-[calc(100dvh-24px)] w-full max-w-md overflow-hidden rounded-2xl border border-white/15 bg-slate-950/92 shadow-[0_18px_0_rgba(2,6,23,0.68),0_38px_80px_rgba(0,0,0,0.45)] backdrop-blur-2xl"
...
function CommentForm({ onClose }) {
  ...
  async function onSubmit(event) {
    ...
    try {
      await submitComment({ ...form, pagePath: location.pathname });
      setStatus('Comment submitted successfully.');
      setForm({ name: '', phone: '', board: '', comment: '' });
    } catch (error) {
      setStatus(error.response?.data?.error || error.message || 'Could not submit comment.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <Panel onClose={onClose}>
      <div className="flex items-center justify-between border-b border-white/10 p-4">
        ...
        <button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-xl border border-white/10 bg-white/5 text-slate-200">
          <X size={18} />
        </button>
      </div>
      <form onSubmit={onSubmit} className="thin-scrollbar max-h-[calc(100dvh-120px)] space-y-4 overflow-y-auto p-4">
        <Field label="Name">
        ...
        <button disabled={submitting} type="submit" className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-br from-cyan-300 via-emerald-300 to-amber-300 font-black text-slate-950 shadow-[0_8px_0_rgba(2,6,23,0.7)] transition active:translate-y-1 disabled:opacity-60">
          <Send size={18} />
...
function AdminPanel({ onClose }) {
  ...
  async function verifyOtp(event) {
    ...
    try {
      ...
      setStatus(requestError.response?.data?.error || requestError.message);
    } finally {
      setLoading(false);
    }
  }
    ...
    <Panel onClose={onClose}>
      ...
      {step === 'phone' ? (
        <form onSubmit={sendOtp} className="space-y-4 p-4">
          ...
          <button disabled={loading} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-br from-amber-300 via-orange-300 to-rose-400 font-black text-slate-950 shadow-[0_8px_0_rgba(2,6,23,0.7)] transition active:translate-y-1">
            <ShieldCheck size={18} />
      ...
      {step === 'comments' ? (
        <div className="thin-scrollbar max-h-[calc(100dvh-120px)] space-y-3 overflow-y-auto p-4">
          {comments.length ? (
            comments.map((comment) => (
              <article key={comment.id} className="rounded-xl border border-white/10 bg-white/[0.06] p-3">
            ...
            <p className="rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">No comments yet.</p>
          )}
        </div>
      ) : null}
    </Panel>
    ...
    <>
      <div className="no-print flex shrink-0 items-center gap-2">
        <motion.button
          ...
          className="inline-flex h-11 items-center gap-2 rounded-xl bg-gradient-to-br from-cyan-300 via-emerald-300 to-lime-300 px-3 text-xs font-black text-slate-950 shadow-[0_7px_0_rgba(2,6,23,0.72),0_15px_28px_rgba(16,185,129,0.22)] transition active:translate-y-1 sm:px-4 sm:text-sm"
        ...
        {showAdminButton ? (
          <button
            ...
            className="inline-flex h-11 items-center gap-2 rounded-xl bg-gradient-to-br from-amber-300 via-orange-300 to-rose-400 px-3 text-xs font-black text-slate-950 shadow-[0_7px_0_rgba(2,6,23,0.72),0_15px_28px_rgba(251,146,60,0.22)] transition hover:-translate-y-0.5 active:translate-y-1 sm:px-4 sm:text-sm"
          >
            <ShieldCheck size={17} />
